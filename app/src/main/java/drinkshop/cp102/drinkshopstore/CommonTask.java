...
public class CommonTask extends AsyncTask<String, Integer, String> {
    ...
    public CommonTask(String url, String outStr) {  //如果要使用ASYNCTASK又確定型別的話可以使用建構子
        this.url = url;
        this.outStr = outStr;
    }

    @Override
    protected String doInBackground(String... params) {   //呼叫execute,這邊接,但用建構子接比較好
    ...
    private String getRemoteData() {
        ...
        try {
            ...
            bw.write(outStr);
              Log.d(TAG, "output: " + outStr); //測試用,記得註解掉
            bw.close();
        ...
        //  Log.d(TAG, "input: " + inStr);  //測試用,記得註解掉
